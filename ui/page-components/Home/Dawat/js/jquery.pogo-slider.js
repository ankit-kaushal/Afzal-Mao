...
(function ($, window, document, undefined) {
  const supportsCSSProp = function (featurename) {
    let feature = false;
  ...
  Plugin.prototype = {
    ...
    _init() {
      ...
      self.$element.find('.pogoSlider-slide').each(function () {
        const children = [];
        let elementTransitionDuration = 0;
        ...
        $(this).find('.pogoSlider-slide-element').each(function () {
          const startTime = parseInt($(this).data('start')) !== undefined ? $(this).data('start') : self.settings.elementTransitionStart;
          const duration = parseInt($(this).data('duration')) || self.settings.elementTransitionDuration;
      ...
      const numImages = self.$element.find('img').length;
      ...
      if (numImages > 0) {
        ...
        self.$element.find('img').one('load', () => {
    ...
    _onSliderReady() {
      ...
      if (self.settings.pauseOnHover) {
        self.$element.on('mouseenter', () => {
          self.pause();
        });

        self.$element.on('mouseleave', () => {
          self.resume();
        });
      }
    ...
    _createDirButtons() {
      ...
      $('<button class="pogoSlider-dir-btn pogoSlider-dir-btn--prev"><i class="fa fa-angle-left"></i></button>').appendTo(self.$element).on('click', () => {
        self.prevSlide();
      });

      $('<button class="pogoSlider-dir-btn pogoSlider-dir-btn--next"><i class="fa fa-angle-right"></i></button>').appendTo(self.$element).on('click', () => {
        self.nextSlide();
      });
    },
    ...
    _createNavigation() {
      ...
      const $navContainer = $('<ul class="pogoSlider-nav"></ul>').appendTo(self.$element);
    ...
    getAppliedProps(el) {
      ...
      const stylesReg = new RegExp('{(.+)}');
      el.matches = el.matches || el.webkitMatchesSelector || el.mozMatchesSelector || el.msMatchesSelector || el.oMatchesSelector;
      const inlineStyles = el.getAttribute('style').replace(/ /g, '').split(';');
      const props = [];

      for (let k = 0; k < inlineStyles.length; k++) {
        const inlineProp = inlineStyles[k].split(':')[0];
      ...
      for (const sheet in styleSheets) {
        if (styleSheets.hasOwnProperty(sheet)) {
          const cssRules = styleSheets[sheet].rules || styleSheets[sheet].cssRules;
          for (const r in cssRules) {
            if (el.matches(cssRules[r].selectorText)) {
              const matchedStyles = stylesReg.exec(cssRules[r].cssText.replace(/ /g, ''));
              if (matchedStyles) {
                const styles = matchedStyles[1].split(';');
                for (let j = 0; j < styles.length; j++) {
                  const prop = styles[j].split(':')[0];
                  // if this prop is not empty and has a value and is not already in the props array
    ...
    _preserveTargetSize() {
      const self = this;
      const unitReg = new RegExp('px|%|em', 'i');
      const numReg = new RegExp('[0-9]*.?[0-9]+');
      const pixelReg = new RegExp('px', 'i');
      let scaleFactor = 1;
      ...
      this.$element.find('.pogoSlider-slide-element').each(function () {
        ...
        const appliedProps = self.getAppliedProps(this);
        const styleObj = {};
        ...
        for (let i = 0; i < appliedProps.length; i++) {
          const cssVal = elementStyles.getPropertyValue(appliedProps[i]);

          if (unitReg.test(cssVal) && numReg.test(cssVal)) {
            // get the number
            const numVal = numReg.exec(cssVal);
            // get the unit
            const unitVal = unitReg.exec(cssVal);
    ...
    _slideTimeout(pauseFor) {
      const self = this;
      let timeoutId;

      timeoutId = self.slideTimeoutId = setTimeout(() => {
    ...
    pause() {
      if (this.settings.autoplay) {
        ...
        for (let i = 0; i < this.slides[this.currentSlideIndex].children.length; i++) {
    ...
    _changeSlide(prevSlideIndex, currentSlideIndex) {
      const self = this;
      ...
      const slideTransition = slideTransitions[self.slides[currentSlideIndex].transition] ? self.slides[currentSlideIndex].transition : 'slide';
      const slideTransitionCallback = slideTransitions[slideTransition].apply(self, [prevSlideIndex, currentSlideIndex]);
    ...
    _onSlideStart(slideIndex) {
      ...
      if (this.paused) {
        for (let i = 0; i < this.slides[slideIndex].children.length; i++) {
    ...
    _slideElementsTransitionIn(slideIndex) {
      for (let i = 0; i < this.slides[slideIndex].children.length; i++) {
        const el = this.slides[slideIndex].children[i];
    ...
    _slideElementsTransitionOut(slideIndex) {
      for (let i = 0; i < this.slides[slideIndex].children.length; i++) {
        const el = this.slides[slideIndex].children[i];
    ...
    _slideCleanup(slideIndex, slideVisible) {
    ...
    _slideElementCleanup(slideIndex) {
      const removePogoSlideElementClasses = function (index, className) {
        return (className.match(/pogoSlider-(?:(?:transition)|(?:animation))(?:-[a-zA-Z0-9]+)?(?:--[a-z]+)?/gi) || []).join(' ');
      };

      const removePogoSlideElementStyles = function (index, style) {
    ...
    _createSlideSlices(slideIndex, rows, cols) {
      const numSlices = cols * rows;
      const sliceWidth = 100 / cols;
      const sliceHeight = 100 / rows;
      const sliceInnerWidth = 100 * cols;
      const sliceInnerHeight = 100 * rows;
      const $el = this.slides[slideIndex].$element;
      ...
      if (slideIndex === this.prevSlideIndex && this.slides[slideIndex].children.length > 0 && timeElapsed < this.slides[slideIndex].elementTransitionDuration) {
        for (let i = 0; i < this.slides[slideIndex].children.length; i++) {
          const animationDelay = `${this.slides[slideIndex].children[i].startTime - timeElapsed}ms`;
      ...
      $el.attr('style', (i, style) => style.replace(/(?:background)[^;]+;/g, ''));

      for (let j = 0; j < numSlices; j++) {
        const colNum = j % rows;
        const rowNum = Math.floor(j / rows);

        const slicePosStyles = `width:${sliceWidth}%;height:${sliceHeight}%;top:${sliceHeight * colNum}%;left:${sliceWidth * rowNum}%;`;
        const sliceInnerPosStyles = `width:${sliceInnerWidth}%;height:${sliceInnerHeight}%;top:-${100 * colNum}%;left:-${100 * rowNum}%;`;
        let sliceInnerBackgroundStyles = '';
        ...
        let el = $el.find('.pogoSlider-slide-slice')
          .last();
        if (j != 0) {
    ...
    _removeSlideSlices(slideIndex) {
      const self = this;
      const $el = self.slides[slideIndex].$element;
    ...
    _generateARandomArray(numItems) {
      const arr = [];
    ...
    slideTransitions: {
      ...
      fade(prevSlideIndex, currentSlideIndex) {
      ...
      slide(prevSlideIndex, currentSlideIndex) {
        let method;
      ...
      verticalSlide(prevSlideIndex, currentSlideIndex) {
        let method;
      ...
      slideLeft(prevSlideIndex, currentSlideIndex) {
        const self = this;
        const currentSlide = self.slides[currentSlideIndex];
      ...
      slideRight(prevSlideIndex, currentSlideIndex) {
        const self = this;
        const currentSlide = self.slides[currentSlideIndex];
      ...
      slideUp(prevSlideIndex, currentSlideIndex) {
        const self = this;
        const currentSlide = self.slides[currentSlideIndex];
      ...
      slideDown(prevSlideIndex, currentSlideIndex) {
        const self = this;
        const currentSlide = self.slides[currentSlideIndex];
      ...
      slideRevealLeft(prevSlideIndex, currentSlideIndex) {
        const self = this;
        const currentSlide = self.slides[currentSlideIndex];
      ...
      slideRevealRight(prevSlideIndex, currentSlideIndex) {
        const self = this;
        const currentSlide = self.slides[currentSlideIndex];
      ...
      slideOverLeft(prevSlideIndex, currentSlideIndex) {
        const currentSlide = this.slides[currentSlideIndex];
      ...
      slideOverRight(prevSlideIndex, currentSlideIndex) {
        const currentSlide = this.slides[currentSlideIndex];
      ...
      expandReveal(prevSlideIndex, currentSlideIndex) {
        const self = this;
        const currentSlide = self.slides[currentSlideIndex];
      ...
      shrinkReveal(prevSlideIndex, currentSlideIndex) {
        const self = this;
        const currentSlide = self.slides[currentSlideIndex];
      ...
      verticalSplitReveal(prevSlideIndex, currentSlideIndex) {
        const self = this;
        const currentSlide = self.slides[currentSlideIndex];
        ...
        const $slices = self.slides[prevSlideIndex].$element.find('.pogoSlider-slide-slice');
      ...
      horizontalSplitReveal(prevSlideIndex, currentSlideIndex) {
        const self = this;
        const currentSlide = self.slides[currentSlideIndex];
      ...
      zipReveal(prevSlideIndex, currentSlideIndex) {
        const self = this;
        const currentSlide = self.slides[currentSlideIndex];
      ...
      barReveal(prevSlideIndex, currentSlideIndex, direction) {
        ...
        const animationDelay = currentSlide.duration / ($slices.length + 1);
        const animationDuration = animationDelay * 2;
      ...
      blocksReveal(prevSlideIndex, currentSlideIndex) {
        const self = this;
        const currentSlide = self.slides[currentSlideIndex];
        ...
        const numRows = Math.round(height / 100); // 100 is the target square size
        const numCols = Math.round(self.$element.width() / 100);
        ...
        const randArr = self._generateARandomArray(numRows * numCols);
      ...
      fold(prevSlideIndex, currentSlideIndex) {
        let method;
      ...
      foldRight(prevSlideIndex, currentSlideIndex) {
        const self = this;
        const currentSlide = self.slides[currentSlideIndex];
        const prevSlide = self.slides[prevSlideIndex];
        ...
        self._createSlideSlices(prevSlideIndex, 1, 2);
        const $prevSlideSlices = prevSlide.$element.find('.pogoSlider-slide-slice');

        self._createSlideSlices(currentSlideIndex, 1, 2);
        const $currentSlideSlices = self.slides[currentSlideIndex].$element.find('.pogoSlider-slide-slice');

        const $bottomLeft = $prevSlideSlices.eq(0);
        // var $bottomRight = $prevSlideSlices.eq(1);
        const $topLeft = $currentSlideSlices.eq(0);
        const $topRight = $currentSlideSlices.eq(1);
      ...
      foldLeft(prevSlideIndex, currentSlideIndex) {
        const self = this;
        const currentSlide = self.slides[currentSlideIndex];
        const prevSlide = self.slides[prevSlideIndex];
        ...
        const $bottomRight = $prevSlideSlices.eq(1);
        const $topLeft = $currentSlideSlices.eq(0);
        const $topRight = $currentSlideSlices.eq(1);
    ...
    compatSlideTransitions: {
      ...
      zipReveal(prevSlideIndex, currentSlideIndex) {
        ...
        const transitionDelay = currentSlide.duration / ($slices.length + 1);
        const transitionDuration = transitionDelay * 2;
